...
| `/admin/crm-analytics?tab=leads` | Route load from CRM dashboard / leads tab | None | Pass after fix | Server-side crash: “Functions are not valid as a child of Client Components” | Moved CRM lead row action menu into client boundary via [components/admin/crm-lead-row-actions.tsx](/Users/biniamhaile/Desktop/Files/elel-events-corrected-styled/components/admin/crm-lead-row-actions.tsx) and updated [components/admin/crm-leads-table.tsx](/Users/biniamhaile/Desktop/Files/elel-events-corrected-styled/components/admin/crm-leads-table.tsx) | Retested in browser and server logs: route now returns HTTP 200 |
